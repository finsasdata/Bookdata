...
data nbitspx;
set bitspx; retain _spx_;
if not missing(spx) then _spx_=spx;
else Spx=_spx_; drop _spx_;
run;
...
%datapull(plot,plot_data.sas7bdat); %datapull(states,states.sas7bdat);
...
data PortRR;
	...
	Period=2;
	if first.ticker then do; /* Assign BGWeights to Annual Sector Returns*/
		tempWeight= weights; 
		Period = Period -1;
	end;
...
/*Generating Portfolio Performance Attribute Report*/
proc print data=Performance_Attrib noobs label style(header)={just=c fontsize=1.8} style(data)={just=c fontsize=1.8} style(grandtot)={just=c fontweight=bold fontsize=1.8}  grandtot_Label='Total' ;
	title ' Portfolio Performance Attribution';
	variables Sector/style(data)={just=l fontweight=bold width=1.8in};
	var BGWeights Returns Sector_P Index_Weights  Index_Returns Sector_I Sector_T Sector_Alpha  
	PAllocation  PSelection  PInteraction ;
	sum BGWeights Index_Weights Sector_P Sector_I PAllocation PSelection Pinteraction;
	footnote H=0.5  'Benchmark is the Russell 3000 Index | Sector tilt = Portfolio overweighting(underweighting) of each sector relative to the index | 
	Sector Alpha = Portfolio Outperformance(underperformance) of each sector relative to the index';
run;
title;
footnote;
...
title 'Timeseries Components of S&P 500 Trading Volume ';
 ...
 series x=date y=volume_d12 /name="Volumec" y2axis   lineattrs=(color=blue) legendlabel= "Trend Cycle Component ";
   ...
   yaxis label='Seasonal and Irregular Components'; y2axis label='Trend Cycle Components';
...
/****************Program 2.31****************/
/* Using Modeling Conditional Variances Using GARCH Specification*/
ods graphics on;
proc autoreg data= corporate_bonds (where=(date>'31dec12'd)) plots=all;
model raaap=/   	/*AAA Bond Indices*/
garch=(p=1,q=1);
...
stationarity=(adf=3,pp=3); 		/*adf-Augmented Dickey-Fuller*/
...
/****************Program 2.33****************/
/*Making Data Stationarity Using Proc Arima*/
ods graphics on;
proc arima data=gdp
 plots=all;
...
/****************Program 2.34****************/
/*Examining Normality in Quarterly Changes in US Real GDP*/
...
title ' Days Per Month with 2% Jumps in the S&P 500 Index';
	...
	Xaxis label = 'Month' ;Yaxis label='Number of Days with Jumps';
