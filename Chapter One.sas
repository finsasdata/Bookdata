...
/******************Program 1.8B****************/
title 'Financial Performance of S&P 500 Companies';
proc tabulate data = NSP500FIN;
	var SPSG EPSG DPRG PEG;
	table SPSG*F=percent8.2  EPSG*F=percent8.2 DPRG*F=percent8.2 PEG, 
	mean stddev median max; /*label SPSG='Sales Per Share Growth Rate' 
	EPSG ='Earnings Per Share Growth Rate' PEG='Price-to-Earnings Growth Rate';*/
run;
title;
/******************Program 1.8C****************/
...
data SSPX;
	...
	do iter=1 to 10; /*number of replication*/
			sumret=0;
		do time = 1 to 120; /*Simulation window*/
			sret =rand("normal",&smean,&ssd);
			sumret=sumret+sret;
			simret=exp(sumret);
			output;
		end;
	end;
	label Simret='Simulated Index Return';
run;

proc print data=SSPX (where=(time=120)) label;
var iter sret sumret simret;
run;
/* To Extract the Descriptive Statistic for the Simulated Returns*/
proc tabulate data=SSPX;
	class iter;
	var  simret;
	table simret,mean*F=percent8.2 stddev*F=percent8.2;
run;
